...
class MyModelApiView(APIView):
    def get_object(self,pk):
        try:
            item=todo.Todo.objects.get(pk=pk)
            if not item.exists():
                return Response({"ereor":"Item not found"},status=status.HTTP_404_NOT_FOUND)
    ...
    def post(self,requset):
        try:
            seralizer=task_serializers.Taskserializers(data=requset.data)
    ...
    def put(self,request,pk):
        try:
            task=todo.Todo.objects.get(pk=pk)
            if not task.exists():
                return Response({"error":"tat key item doesnot exist in db"},status=status.HTTP_404_NOT_FOUND)
            serializer=task_serializers.Taskserializers(task,data=request.data)
            if serializer.is_valid():
                serializer.save()
                return Response(serializer.data,status=status.HTTP_200_OK)
